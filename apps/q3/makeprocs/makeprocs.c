...
void main (int argc, char *argv[])
{
  ...
  if ((s_emptyslots = sem_create(BUF_SIZE)) == SYNC_FAIL) {
    Printf("Bad sem_create in "); Printf(argv[0]); Printf("\n");
    Exit();
  }
  ...
  for(i=0; i<numprocs; i++) {
    process_create(PRODUCER_FILE_TO_RUN, h_mem_str, s_procs_completed_str, s_fullslots_str, s_emptyslots_str, buffer_lock_str, NULL);
    //Printf("Producer %d created\n", i);
    process_create(CONSUMER_FILE_TO_RUN, h_mem_str, s_procs_completed_str, s_fullslots_str, s_emptyslots_str, buffer_lock_str, NULL);
