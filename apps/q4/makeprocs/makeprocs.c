...
void main (int argc, char *argv[])
{
  ...
  char c_notFull_str[10];
  char c_notEmpty_str[10];
  lock_t buffer_lock;
  ...
  if ((c_notFull = cond_create(buffer_lock)) == SYNC_FAIL) {
    Printf("Bad cond_create in "); Printf(argv[0]); Printf("\n");
    Exit();
  } else {
    Printf("Created c_notFull %d\n", c_notFull);
  }

  if ((c_notEmpty = cond_create(buffer_lock)) == SYNC_FAIL) {
    Printf("Bad cond_create in "); Printf(argv[0]); Printf("\n");
    Exit();
  } else {
    Printf("Created c_notEmpty %d\n", c_notEmpty);
  }

  //Create argument strings for child processes
  ditoa(h_mem, h_mem_str);
  ditoa(s_procs_completed, s_procs_completed_str);
  ditoa(c_notFull, c_notFull_str);
  ditoa(c_notEmpty, c_notEmpty_str);
